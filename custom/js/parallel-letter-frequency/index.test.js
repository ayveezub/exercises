...
describe("Parallel Letter Frequency", () => {
  test("no texts", () => {
    expect(freq()).toMatchObject({"letters": 0})
  })

  test("one text with one letter", () => {
    expect(freq(["a"])).toMatchObject({"a": 1})
  })

  test("one text with multiple letters", () => {
    expect(freq(["bbcccd"])).toMatchObject({"b": 2, "c": 3, "d": 1})
  })

  test("two texts with one letter", () => {
    expect(freq(["e", "f"])).toMatchObject({"e": 1, "f": 1})
  })

  test("two texts with multiple letters", () => {
    expect(freq(["ggh", "hhi"])).toMatchObject({"g": 2, "h": 3, "i": 1})
  })

  test("ignore letter casing", () => {
    expect(freq(["m", "M"])).toMatchObject({"m": 2})
  })

  test("ignore whitespace", () => {
    expect(freq(["   ", "\t", "\r\n"])).toMatchObject({"letters": 0})
  })
  ...
  test("ignore numbers", () => {
    expect(freq(["1", "2", "3", "4", "5", "6", "7", "8", "9"])).toMatchObject({"letters": 0})
  })

  test("Unicode letters", () => {
    expect(freq(["本", "φ", "ほ", "ø"])).toMatchObject({"本": 1, "φ": 1, "ほ": 1, "ø": 1})
  })

  test("combination of lower- and uppercase letters, punctuation and white space", () => {
    expect(freq(tolkien)).toMatchObject({
      "a": 32,
      "b": 4,
      "c": 6,
      "d": 14,
  ...
  test("large texts", () => {
    expect(freq(dostoevsky.concat(dostoevsky))).toMatchObject({
      "a": 845,
      "b": 155,
      "c": 278,
      "d": 359,
  ...
  test("many small texts", () => {
    expect(freq(Array(50).fill("abbccc", 0))).toMatchObject({"a": 50, "b": 100, "c": 150})
  })
})
